...
import { PanelRequestData, panelRequestDataResolver } from '@/core/models/validationSchemas/panelRequestData'
...
export const EditPanelForm: React.FC<{panel?:listOnePanelResponse | null}> = (data) => {
  const { panelLayouts, passwordCallConfigurations, panelId, updatePanel } = usePanelAndTotem();  
  ...
  const locationsSelect = session.data?.user.access.find((item) => item.id === session.data?.user?.selectedContract?.id)
