...
import { editPanelRequestData, PanelRequestData } from "@/core/models/validationSchemas/panelRequestData";
import { EditServiceRequestData, ServiceRequestData } from "@/core/models/validationSchemas/serviceRequestData";
...
interface PanelAndTotemStoreData { 
    ...
    handleUpdateService: (control:string, value:any, id:string) => void 
    handleUpdateClassification: (control:string, value:any, id:string) => void
    handleConnectServiceToPanel: (panelId:string, serviceId:string, active:boolean) => void
    ...
    createServicePasswordWithPanel: (panelId:string, data:ServiceRequestData) => void
    updateServicePassword: (id:string, data:EditServiceRequestData) => void
    findServicePasswordById: (id:string) => void
...
export const PanelAndTotemProvider: React.FC<PropsWithChildren> = ({ children }) => {
    const { api } = useApi() 
   ...
   const fecthPanel = useCallback(async () => {
    ...
    async function handleActivePannel(id:string, checked:boolean){
      try { 
        await paramsV2Api.put(`panel/${id}`, 
          { active: checked},{
            headers: { Authorization: `Bearer ${session.data?.user.token}` }
          })  
    ...
    async function handleUpdateService(control:string, value:any, id:string){
      try {  
        await api.patch(`service-password/${id}`, 
          { [control]: value },{
            headers: { Authorization: `Bearer ${session.data?.user.token}` }
          })    
    ...
    async function handleUpdateClassification(control:string, value:any, id:string){
      try {  
        await api.patch(`service-classification/${id}`, 
          { [control]: value },{
            headers: { Authorization: `Bearer ${session.data?.user.token}` }
          })     
          queryClient.invalidateQueries({
    ...
    async function handleConnectServiceToPanel(panelId:string, serviceId:string, active:boolean){
      try {  
        if(active){
          await paramsV2Api.post(`panel/${panelId}/service-password/${serviceId}/add`, 
            {},{
              headers: { Authorization: `Bearer ${session.data?.user.token}` }
            })  
          toast.success('Serviço conectado ao painel com sucesso')  
        }else{
          await paramsV2Api.delete(`panel/${panelId}/service-password/${serviceId}/remove`, 
            {
              headers: { Authorization: `Bearer ${session.data?.user.token}` }
            })    
          queryClient.invalidateQueries({
    ...
    async function createClassification(data:ClassificationRequestData){
      try { 
        ...
        await api.post(`service-classification`, 
    ...
    async function createServicePasswordWithPanel(panelId:string, data:ServiceRequestData){
      try {  
        await api.post(`service-password/panel/${panelId}`, 
          {
    ...
    async function findServicePasswordById(id:string){
      try {  
    ...
    async function updateServicePassword(id:string, data: EditServiceRequestData){
      try {  
        await api.patch(`service-password/${id}`, 
          {
            description: data.name,
